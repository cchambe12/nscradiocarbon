...
\begin{align*}
y_i &= \alpha_{season[i]} + \beta_{increment1-2_{season[i]}} + \beta_{increment2-3_{season[i]}} + \beta_{increment3-4_{season[i]}} + \beta_{increment4-8_{season[i]}} + \beta_{increment8-pith_{season[i]}} + \epsilon_i\\,
\end{align*}
\begin{align*}
...
ringtot<-read.csv("../analyses/output/ringtotal_tab.csv", header=TRUE)
...
print(mod.table, include.rownames=TRUE, hline.after = c(-1,0,6),caption.placement="top",size="\\footnotesize",sanitize.rownames.function=identity)
...
difftot<-read.csv("../analyses/output/difftotal_tab.csv", header=TRUE)
...
ringsug<-read.csv("../analyses/output/ringsugar_tab.csv", header=TRUE)
...
diffsug<-read.csv("../analyses/output/diffsugar_tab.csv", header=TRUE)
...
ringstar<-read.csv("../analyses/output/ringstarch_tab.csv", header=TRUE)
...
diffstar<-read.csv("../analyses/output/diffstarch_tab.csv", header=TRUE)
...
colnames(ringradio) <- c('mean', '25%', '75%', '2.5%', '97.5%' )

mod.table <- xtable(diffstar, caption="\\textbf{Estimates from ring-porous radiocarbon ages}. Using a model testing the effects of organ on radiocarbon age for ring-porous species, results in muted variation in age across organs. We present posterior means, as well as 50 percent and 97.5 percent uncertainty intervals from the model.", 
        label="tab:lat",align=c(
               "|p{0.14\\textwidth}|",
               "p{0.06\\textwidth}|",
               "p{0.06\\textwidth}|",
...
diffradio<-read.csv("../analyses/output/diff_radiocarbon_table.csv", header=TRUE)
rownames(diffradio)<-diffradio$X
  diffradio<-diffradio[,-1]      
colnames(diffradio) <- c('mean', '25%', '75%', '2.5%', '97.5%' )

mod.table <- xtable(diffstar, caption="\\textbf{Estimates from diffuse-porous radiocarbon ages}. Using a model testing the effects of organ on radiocarbon age for diffuse-porous species, results in slight greater variation in age across organs. We present posterior means, as well as 50 percent and 97.5 percent uncertainty intervals from the model.", 
        label="tab:lat",align=c(
               "|p{0.14\\textwidth}|",
               "p{0.06\\textwidth}|",
               "p{0.06\\textwidth}|",
